...
alert(`Welcome to "GUESS THAT NUMBER!" Please clikck "OK" to start the game`)
...
while (restartGame) {
    ...
    // Using parseInt to attempt to convert the user's reponse into a number value
    ...
    while (true) {
        ...
        while (!guess || guess < 1 || guess > rangeNum){
            guess = parseInt(prompt(`Please enter the number from 1 to ${rangeNum}`));
        }
        ...
        // Checks if users has any attemps left. If not, then the game ends and the number is displayed to the user.
        } else if (attempts === 0){
            alert(`Sorry, but you have run out of attemps :(The number was ${randomNum})`);
            break;
            ...
            // The only other possibility is the user's guess was too high so the user is promptes again
        }else {
            guess = prompt(`Too high. You have ${attempts} attempt(s) left.`);
        }
    }

    // Prompts user with option
    ...
    while(true) {
        ...
        if (playAgain.toUpperCase() === 'N'){
            // Alert the user that the game is over and game does NOT restart
